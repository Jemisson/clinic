...
import type { PatientPhotoGroups, PatientPhotoLabel } from "@/types/patients.photos"
...
export default function PatientPhotosCard({ patientId }: Props) {
  const { data: groups, error, isLoading, mutate } = useSWR<PatientPhotoGroups>(
  ...
  const [lightboxItems, setLightboxItems] = React.useState<LightboxItem[]>([])
  const [lightboxIndex, setLightboxIndex] = React.useState(0)

  const [confirmOpen, setConfirmOpen] = React.useState(false)
  const [deleting, setDeleting] = React.useState(false)
  const [targetToDelete, setTargetToDelete] = React.useState<{
    id: number
    dateKey: string
    title?: string | null
    label?: string
    ...
    <>
      <Card className="border-none shadow-none p-0">
        <CardContent className="pt-4 px-0 text-muted-foreground">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xl font-medium text-primary">Fotos do paciente</h3>

            <PatientPhotoDialog patientId={patientId} onSaved={handleSaved}>
          ...
          {error && (
            <p className="text-destructive">
              Erro: {(error as any)?.message ?? "Falha ao carregar fotos"}
            </p>
          )}

          {!isLoading && !error && (!groups || orderedDates.length === 0) && (
            <p className="text-muted-foreground">Em breve: galeria de fotos do paciente.</p>
          )}
                  ...
                  <section key={date} className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                      {itemsSorted.map((p, idx) => {
                        const thumb = p.image_variants?.thumb ?? p.image_url ?? ""
                        const label = PATIENT_PHOTO_LABEL_SHORT[p.label]
                        const alt = p.title ?? label
                        ...
                        return (
                          <div key={p.id} className="rounded-md border p-2">
                            ...
                            <div className="mt-2 text-xs text-muted-foreground">
                              ...
                              {p.captured_at && (
                                <span> — {dayjs(p.captured_at).format("HH:mm")}</span>
                              )}
                            </div>
                          </div>
                        )
      ...
      <ConfirmDialog
        ...
        deactivateDescription={
          <>
            Tem certeza que deseja <b>excluir</b> esta foto? Essa ação não pode ser desfeita depois.
          </>
        }
        loading={deleting}
