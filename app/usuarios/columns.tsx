...
export const columns = ({ onRequestStatusChange, onEdit }: ColumnsHandlers): ColumnDef<ProfileUserData>[] => [
    ...
    header: ({ column }) => {
      ...
      return (
        <Button variant="ghost" onClick={() => column.toggleSorting(sorted === "asc")}>
          ...
          {sorted === "asc" ? <ArrowUp /> :
           sorted === "desc" ? <ArrowDown /> : <ChevronsUpDown />}
        </Button>
      )
    },
    cell: ({ row }) => {
  ...
  },
  {
    ...
    cell: ({ row }) => <span className="capitalize">{row.original.attributes.role}</span>
  },
  {
    id: "sector",
    accessorKey: "sector",
    ...
    header: ({ column }) => {
      ...
      return (
        <Button variant="ghost" onClick={() => column.toggleSorting(sorted === "asc")}>
          ...
          {sorted === "asc" ? <LucideIcons.ArrowUp /> :
           sorted === "desc" ? <LucideIcons.ArrowDown /> : <LucideIcons.ChevronsUpDown />}
        </Button>
      )
    },
    cell: ({ row }) => {
      ...
      return (
        <Badge variant={"outline"} className="border-none">
          <span className={`size-1.5 rounded-full ${status === "active" ? "bg-green-500" : "bg-red-500"}`} />
      ...
      return (
        <div className="flex gap-1">
          ...
          <Button
            ...
            onClick={() => onRequestStatusChange(user, isActive ? "inactive" : "active")}
          >
            {isActive
