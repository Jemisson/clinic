...
const RESOURCE = (patientId: string | number) =>
  `/patients/${patientId}/patient_photos`
...
export const PatientPhotosService = {
  list: async (
    ...
    const { data } = await api.get<PatientPhotosResponse>(RESOURCE(patientId), {
      params,
    })
  ...
  show: async (patientId: string | number, id: string | number) => {
    const response = await api.get<PatientPhotoShowResponse>(
      `${RESOURCE(patientId)}/${id}`,
    )
